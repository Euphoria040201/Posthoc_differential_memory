...
if HAS_QWEN3_MOE:
    SUPPORTED_BASE_ATTENTION_TYPES += (Qwen3MoeAttention,)
...
def parse_moe_schedule(spec: str) -> tuple[tuple[int, int, int], ...]:
    """Parse "L:W:R,L2:W2:R2" -> sorted tuple of (min_layer, write_k, read_k)."""
    ...
    for part in spec.split(","):
        ...
        if not part:
            continue
        l, w, r = (int(x) for x in part.split(":"))
        entries.append((l, w, r))
    return tuple(sorted(entries))


VALID_DELTA_PROJ_MODES = ("dense", "head_aligned")


def normalize_delta_proj_mode(mode: str) -> str:
    normalized = str(mode).strip().lower()
    if normalized not in VALID_DELTA_PROJ_MODES:
        raise ValueError(
            f"Unsupported delta_proj_mode: {mode!r}; expected one of {VALID_DELTA_PROJ_MODES}"
        )
    return normalized
...
class HFDeltaMemConfig:
    ...
    delta_proj_mode: str = "dense"
    memory_moe_routing: bool = False
    memory_moe_top_k: int = 2
    memory_moe_router_temp: float = 1.0
    # Per-layer depth schedule for write/read top-k. Format: "L:W:R,..." meaning from layer L
    # onward use write top-W, read top-R. A side with k>=num_state_heads is dense (no routing).
    # Empty => uniform memory_moe_top_k for both sides on every layer.
    memory_moe_schedule: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "delta_proj_mode", normalize_delta_proj_mode(self.delta_proj_mode)
        )
        object.__setattr__(self, "memory_moe_routing", bool(self.memory_moe_routing))
        object.__setattr__(self, "memory_moe_top_k", int(self.memory_moe_top_k))
        object.__setattr__(self, "memory_moe_router_temp", float(self.memory_moe_router_temp))
        if self.memory_moe_routing and self.memory_moe_top_k < 1:
            raise ValueError("memory_moe_top_k must be >= 1")
        if self.memory_moe_router_temp <= 0:
            raise ValueError("memory_moe_router_temp must be > 0")
        object.__setattr__(
            self,
            "state_update_mode",
    ...
    def from_dict(cls, data: dict) -> "HFDeltaMemConfig":
        # Backward compat: adapters trained before certain knobs were archived may
        # carry config keys that are no longer constructor fields (e.g. slot_memory,
        # slot_count, slot_router_temp, head_decay_biases). Drop unknown keys so old
        # adapters keep loading; warn so a non-inert legacy value isn't silently ignored.
        known_fields = set(cls.__dataclass_fields__)
        legacy_keys = [k for k in data if k not in known_fields]
        if legacy_keys:
            import warnings

            data = {k: v for k, v in data.items() if k in known_fields}
            warnings.warn(
                "Ignoring legacy Delta-Mem config keys not present in the current "
                f"HFDeltaMemConfig: {sorted(legacy_keys)}",
                stacklevel=2,
            )
...
class DeltaMemAttention(nn.Module):
    def __init__(self, base: Qwen3Attention | SmolLM3Attention, config: HFDeltaMemConfig) -> None:
        ...
        self.num_attention_heads = base.q_proj.out_features // self.head_dim
        # Head-Aligned DeltaMem (v1a): each state head is bound 1:1 to a GQA KV group and
        # only injects into that group's attention heads (block-diagonal readout), instead of
        # the dense readout that mixes all states into all heads.
        self.delta_proj_mode = config.delta_proj_mode
        self.head_aligned_delta = self.delta_proj_mode == "head_aligned"
        if self.head_aligned_delta:
            if not self.multi_head_state:
                raise ValueError(
                    "delta_proj_mode='head_aligned' requires num_state_heads > 1"
                )
            if self.num_state_heads != self.num_key_value_heads:
                raise ValueError(
                    "delta_proj_mode='head_aligned' requires num_state_heads == "
                    f"num_key_value_heads (got {self.num_state_heads} vs {self.num_key_value_heads})"
                )
            if self.num_attention_heads % self.num_state_heads != 0:
                raise ValueError(
                    "delta_proj_mode='head_aligned' requires num_attention_heads divisible "
                    f"by num_state_heads (got {self.num_attention_heads} / {self.num_state_heads})"
                )
            if "o" in self.active_delta_heads:
                raise ValueError(
                    "delta_proj_mode='head_aligned' (v1a) supports delta_heads subset of "
                    "{q,k,v}; the 'o' head needs pre-o_proj relocation (v1b). Use "
                    "--delta-heads q,k,v (or drop o)."
                )
            self.num_q_per_group = self.num_attention_heads // self.num_state_heads
        ...
        # MoE-DeltaMem (v2): per-token learned router over the num_state_heads memory states.
        # Write = sparse (top-k states updated, rest frozen); read = soft (top-k softmax-weighted
        # per-state reads feeding the existing dense readout, so cross-head mixing is preserved).
        # Dense MSW is the uniform-routing special case; hard head-aligned the one-hot special case.
        self.memory_moe_routing = config.memory_moe_routing
        self.memory_moe_top_k = min(config.memory_moe_top_k, self.num_state_heads)
        self.memory_moe_router_temp = config.memory_moe_router_temp
        self.moe_schedule = parse_moe_schedule(config.memory_moe_schedule)
        if self.memory_moe_routing:
            if not self.multi_head_state:
                raise ValueError("memory_moe_routing requires num_state_heads > 1")
            self.moe_router_proj = nn.Parameter(torch.empty(self.num_state_heads, hidden_size))
            self.moe_router_bias = nn.Parameter(torch.zeros(self.num_state_heads))

        if self.head_aligned_delta:
            # Grouped block-diagonal weights: each attention/KV head reads ONLY its own
            # rank-r state, via a per-head (head_dim, rank) block. ~num_state_heads x fewer
            # params/FLOPs than the dense (out_features, rank*num_state_heads) projection.
            self.delta_q_proj = nn.Parameter(
                torch.empty(self.num_attention_heads, self.head_dim, self.rank)
            )
            self.delta_k_proj = nn.Parameter(
                torch.empty(self.num_key_value_heads, self.head_dim, self.rank)
            )
            self.delta_v_proj = nn.Parameter(
                torch.empty(self.num_key_value_heads, self.head_dim, self.rank)
            )
        else:
        ...
        self.last_moe_route_probs: torch.Tensor | None = None
    ...
    def _init_delta_head_grouped(self, head: nn.Parameter, base_weight: torch.Tensor) -> None:
        # head: (num_heads, head_dim, rank); base_weight: (num_heads*head_dim, hidden).
        ...
        with torch.no_grad():
            ...
            if slice_width == 0:
                return
            per_head = base_weight.view(num_heads, self.head_dim, base_weight.shape[1])
            base_slice = per_head[:, :, :slice_width].detach().clone().float()
            base_slice = F.normalize(base_slice, dim=1, eps=1e-6)  # normalize over head_dim
            head[:, :, :slice_width].copy_((base_slice * self.online_gain).to(head.dtype))
    ...
    def reset_parameters(self) -> None:
        ...
        if self.head_aligned_delta:
            self._init_delta_head_grouped(self.delta_q_proj, self.base.q_proj.weight)
            self._init_delta_head_grouped(self.delta_k_proj, self.base.k_proj.weight)
            self._init_delta_head_grouped(
                self.delta_v_proj,
        ...
        if self.memory_moe_routing:
            nn.init.normal_(self.moe_router_proj, std=0.02)
            nn.init.zeros_(self.moe_router_bias)
    ...
    def reset_state(self) -> None:
        ...
        self.last_moe_route_probs = None
    ...
    def _project_delta_head(
        ...
        if head_name not in self.active_delta_heads:
            return None
        if self.head_aligned_delta and head_name in ("q", "k", "v"):
            projected = self._project_delta_head_aligned(reads, weight, head_name)
        else:
    ...
    def _project_delta_head_aligned(
        self,
        reads: torch.Tensor,
        weight: torch.Tensor,
        head_name: str,
    ) -> torch.Tensor:
        # reads: (..., num_state_heads*rank), head-major. weight: (num_heads, head_dim, rank).
        lead = reads.shape[:-1]
        m = reads.view(*lead, self.num_state_heads, self.rank)
        if head_name == "q":
            # each of the num_q_per_group query heads in group g reads state g
            m = m.repeat_interleave(self.num_q_per_group, dim=-2)  # (..., num_attention_heads, rank)
        # k/v: one query per KV group, m already (..., num_key_value_heads, rank)
        projected = torch.einsum("...hr,hdr->...hd", m.to(weight.dtype), weight)
        return projected.reshape(*lead, weight.shape[0] * self.head_dim)

    def _resolve_moe_k(self) -> tuple[int, int]:
        # per-layer (write_k, read_k) from the depth schedule (else uniform top_k)
        wk = rk = self.memory_moe_top_k
        for (lyr, w, r) in self.moe_schedule:
            if self.layer_idx >= lyr:
                wk, rk = w, r
        H = self.num_state_heads
        return min(wk, H), min(rk, H)

    def _compute_moe_gates(
        self,
        hidden_states: torch.Tensor,
        token_mask: Optional[torch.Tensor],
    ) -> tuple[torch.Tensor | None, torch.Tensor | None]:
        # Per-token router over the num_state_heads states, with per-layer write/read top-k.
        # read_gate: soft softmax weights within read-top-k (None => dense read, all states).
        # write_mask: binary write-top-k indicator (None => dense write, all states updated).
        H = self.num_state_heads
        wk, rk = self._resolve_moe_k()
        logits = F.linear(hidden_states, self.moe_router_proj, self.moe_router_bias)
        probs = torch.softmax(logits.float() / self.memory_moe_router_temp, dim=-1)
        self.last_moe_route_probs = probs.detach()
        if wk < H:
            w_idx = torch.topk(probs, wk, dim=-1).indices
            w_mask = torch.zeros_like(probs).scatter_(-1, w_idx, 1.0)
            write_mask = w_mask.to(hidden_states.dtype)
            self.last_moe_aux = self._moe_load_balance(probs, w_mask, token_mask)
        else:
            write_mask = None  # dense write
            self.last_moe_aux = None
        if rk < H:
            r_idx = torch.topk(probs, rk, dim=-1).indices
            r_mask = torch.zeros_like(probs).scatter_(-1, r_idx, 1.0)
            read_gate = (probs * r_mask).to(hidden_states.dtype)
        else:
            read_gate = None  # dense read
        self.last_moe_write_mask = write_mask.detach() if write_mask is not None else None
        self.last_moe_read_gate = read_gate.detach() if read_gate is not None else None
        return read_gate, write_mask

    def _moe_load_balance(
        self,
        probs: torch.Tensor,
        mask: torch.Tensor,
        token_mask: Optional[torch.Tensor],
    ) -> torch.Tensor:
        # Switch-style aux loss: N * sum_e (fraction routed to e) * (mean router prob for e).
        # Minimized when routing is balanced across states; encourages all states to be used.
        if token_mask is not None:
            m = token_mask.to(probs.dtype).unsqueeze(-1)
            denom = m.sum().clamp_min(1.0)
            importance = (probs * m).sum(dim=(0, 1)) / denom
            load = (mask * m).sum(dim=(0, 1)) / denom
        else:
            importance = probs.mean(dim=(0, 1))
            load = mask.mean(dim=(0, 1))
        return self.num_state_heads * (importance * load).sum()

    def _apply_read_route(self, reads: torch.Tensor, read_gate: torch.Tensor) -> torch.Tensor:
        # reads: (B, T, num_state_heads*rank) head-major; read_gate: (B, T, num_state_heads).
        lead = reads.shape[:-1]
        gated = reads.view(*lead, self.num_state_heads, self.rank) * read_gate.unsqueeze(-1)
        return gated.reshape(*lead, self.state_read_dim)
    ...
    def _memory_affine_scan(
        ...
        token_mask: Optional[torch.Tensor] = None,
        head_write_gate: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.multi_head_state:
            if head_write_gate is not None:
                # MoE sparse write: state e is frozen on tokens where its gate is 0
                # (keep=1, erase=write=0); written normally where the gate is 1.
                g = head_write_gate.unsqueeze(-1).to(keep_seq.dtype)
                keep_seq = 1.0 - g + g * keep_seq
                erase_seq = g * erase_seq
    ...
    def forward(
        ...
        **kwargs,
        ...
        moe_write_mask = None
        if self.memory_moe_routing:
            moe_read_gate, moe_write_mask = self._compute_moe_gates(hidden_states, token_mask)
        else:
            self.last_moe_aux = None
            self.last_moe_route_probs = None
        stats_beta = beta_seq
        ...
        if self.write_enabled:
            ...
            if write_hidden is not None and write_mask is not None:
                ...
                state, reads = self._memory_affine_scan(
                    ...
                    head_write_gate=moe_write_mask,
                )
        else:
            reads = self._token_state_reads(
                state,
        ...
        if moe_read_gate is not None:
            reads = self._apply_read_route(reads, moe_read_gate)
...
def get_delta_mem_moe_load_balance(model: nn.Module) -> torch.Tensor:
    """Mean MoE router load-balance aux loss across layers (0 if routing is off)."""
    terms = []
    for _, module in iter_delta_mem_modules(model):
        if getattr(module, "last_moe_aux", None) is not None:
            terms.append(module.last_moe_aux)
    if terms:
