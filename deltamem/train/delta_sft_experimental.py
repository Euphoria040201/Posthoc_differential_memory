...
from deltamem.core.delta import (
    ...
    get_delta_mem_moe_load_balance,
...
class DeltaMemTrainer(Trainer):
    def __init__(
        ...
        *args,
        ...
        memory_moe_balance_weight: float = 0.0,
        ...
        **kwargs,
        ...
        self.memory_moe_balance_weight = memory_moe_balance_weight
    ...
    def compute_loss(
        ...
        if self.write_sparsity_weight > 0:
            ...
            self._last_write_sparsity_loss = 0.0
        if self.memory_moe_balance_weight > 0:
            moe_balance_loss = get_delta_mem_moe_load_balance(model)
            loss = loss + self.memory_moe_balance_weight * moe_balance_loss
            if isinstance(outputs, dict):
                outputs = dict(outputs)
                outputs["moe_balance_loss"] = moe_balance_loss.detach()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--delta-proj-mode",
        default="dense",
        choices=["dense", "head_aligned"],
        help=(
            "Delta readout injection: 'dense' (mix all state heads into all attention "
            "heads) or 'head_aligned' (block-diagonal: state head g -> KV group g only; "
            "requires num_state_heads == num_key_value_heads and delta_heads subset of q,k,v)"
        ),
    )
    parser.add_argument(
        "--memory-moe-routing",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="MoE-DeltaMem: sparse top-k learned routing over the num_state_heads states.",
    )
    parser.add_argument("--memory-moe-top-k", type=int, default=2)
    parser.add_argument("--memory-moe-router-temp", type=float, default=1.0)
    parser.add_argument(
        "--memory-moe-schedule",
        default="",
        help="Per-layer write/read top-k schedule, e.g. '0:8:8,22:2:2' (dense below L22, top-2 from L22).",
    )
    parser.add_argument(
        "--memory-moe-balance-weight",
        type=float,
        default=0.01,
        help="Weight on the Switch-style router load-balance aux loss.",
    )
    parser.add_argument(
        "--delta-o-rmsnorm",
        action=argparse.BooleanOptionalAction,
...
def main() -> None:
    ...
    delta_config = HFDeltaMemConfig(
        ...
        delta_proj_mode=args.delta_proj_mode,
        memory_moe_routing=args.memory_moe_routing,
        memory_moe_top_k=args.memory_moe_top_k,
        memory_moe_router_temp=args.memory_moe_router_temp,
        memory_moe_schedule=args.memory_moe_schedule,
    ...
    trainer = DeltaMemTrainer(
        ...
        memory_moe_balance_weight=args.memory_moe_balance_weight,
