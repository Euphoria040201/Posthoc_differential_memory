...
if triton is not None and tl is not None:
    ...
    ROUND_STATE: tl.constexpr,
        ...
        while t < seq_len:
            ...
            if ROUND_STATE:
                # Round the running state to bf16 each step to match the (numerically
                # stable) torch reference recurrence; fp32 accumulation overflows to
                # NaN when extrapolating far beyond the training write window.
                updated = updated.to(tl.bfloat16).to(tl.float32)
...
class _TritonAffineScanFn(torch.autograd.Function):
    ...
    def forward(
        ...
        round_state = os.environ.get("DELTA_MEM_SCAN_BF16_STATE", "0") == "1"
