...
def _manual_merge_lora(model, adapter_dir) -> None:
    """Merge a PEFT LoRA adapter into the base weights WITHOUT peft.from_pretrained
    (which is broken on this peft/transformers combo). delta_W = (alpha/r) * B @ A."""
    import re
    ...
    sd = _load_file(str(adapter_dir / "adapter_model.safetensors"))
    pairs: dict[str, dict[str, torch.Tensor]] = {}
    for key, value in sd.items():
        match = re.match(r"(.*)\.lora_(A|B)\.weight$", key)
        if match:
            pairs.setdefault(match.group(1), {})[match.group(2)] = value
    merged = 0
    for base_key, ab in pairs.items():
        if "A" not in ab or "B" not in ab:
            continue
        mod_path = base_key.replace("base_model.model.", "")
        ...
        merged += 1
    if merged == 0:
        raise RuntimeError(f"No LoRA pairs merged from {adapter_dir}")
...
def load_base_model(
    *,
    ...
    # ADAPTED (moe-ttt-baselines): DELTAMEM_DEVICE_MAP=auto shards a large MoE across visible
    # GPUs (model-parallel) so 30B + long δ-mem replay fits; default unchanged.
    import os as _os
    _kw = {}
    if _os.environ.get("DELTAMEM_DEVICE_MAP") == "auto":
        _kw["device_map"] = "auto"
        # Reserve activation headroom on GPU0 (holds embedding + long-sequence activations):
        # give it the LEAST model so the long δ-mem history forward fits. DELTAMEM_MAX_MEMORY
        # = comma list "0:20GiB,1:72GiB,2:72GiB".
        _mm = _os.environ.get("DELTAMEM_MAX_MEMORY")
        if _mm:
            _kw["max_memory"] = {(int(k) if k.strip().isdigit() else k.strip()): v.strip()
                                 for k, v in (p.split(":") for p in _mm.split(","))}
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--context-copy-bias", type=float, default=0.0,
                        help="Logit bias added to tokens present in the prompt context (extractive nudge).")
    parser.add_argument("--terse-prompt", action="store_true",
                        help="Replace the official QA prompt with a stronger 'answer in 1-4 words only' instruction.")
    parser.add_argument("--force-answer", action="store_true",
                        help="Keep official short-answer framing but forbid refusals; force a best-guess answer.")
    parser.add_argument("--no-refusal-hint", action="store_true",
                        help="Drop the 'if you don't know, don't share false information' sentence from the system prompt.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Plain PEFT LoRA adapter to merge into the base model before the base (full_history) eval.",
    )
    parser.add_argument(
        "--peft-adapter-dir",
        type=Path,
        default=None,
        help="Generic PEFT adapter (PromptTuning/PrefixTuning/etc.) attached via PeftModel.from_pretrained "
        "onto the base model before the base (full_history) eval. Use for non-LoRA adapters that cannot "
        "be weight-merged.",
    )
    parser.add_argument(
        "--delta-conditions",
...
_CONTEXT_COPY_ALPHA = 0.0


class _ContextCopyBias:
    """Logit bias toward tokens that appear in the prompt context (extractive nudge).
    Boosts content tokens from the conversation so the answer reuses gold-matching words
    (F1 rewards exact context words over paraphrases)."""

    def __init__(self, context_ids, vocab_size: int, alpha: float, skip_ids=None):
        device = context_ids.device
        B = context_ids.shape[0]
        self.alpha = alpha
        self.mask = torch.zeros(B, vocab_size, dtype=torch.bool, device=device)
        for b in range(B):
            self.mask[b, context_ids[b]] = True
        if skip_ids:
            self.mask[:, list(skip_ids)] = False

    def __call__(self, input_ids, scores):
        return scores + self.alpha * self.mask.to(scores.dtype)[:, : scores.shape[-1]]
            ...
            with torch.inference_mode():
                ...
                if _CONTEXT_COPY_ALPHA != 0.0:
                    ...
                    generate_kwargs["logits_processor"] = [
                        _ContextCopyBias(input_ids, vocab, _CONTEXT_COPY_ALPHA, skip_ids={s for s in skip if s is not None})
                    ]
                if do_sample:
                    generate_kwargs["top_k"] = OFFICIAL_TOP_K if top_k in (None, 0) else int(top_k)
                    generate_kwargs["top_p"] = OFFICIAL_TOP_P if top_p is None else float(top_p)
...
def main() -> None:
    args = parse_args()
    global _CONTEXT_COPY_ALPHA
    _CONTEXT_COPY_ALPHA = float(args.context_copy_bias)
    if getattr(args, "no_refusal_hint", False):
        import deltamem.eval.locomo_protocol as _lp
        _lp.OFFICIAL_SYSTEM_PROMPT = (
    ...
    if getattr(args, "force_answer", False):
        import deltamem.eval.locomo_protocol as _lp
        _lp.OFFICIAL_QA_PROMPT = (
            ...
            "from the conversations whenever possible. Always give your single best answer even if "
            "it is not stated explicitly — infer the most likely answer. NEVER reply that the "
            "information is not available, not mentioned, or unknown.\n\nQuestion: {}"
        )
    if getattr(args, "terse_prompt", False):
        import deltamem.eval.locomo_protocol as _lp
        _lp.OFFICIAL_QA_PROMPT = (
            "Based on the above conversations, answer the question directly without adding "
            "unnecessary descriptions or extra details. Whenever possible, use the exact words "
            "that appear in the conversations.\n\n"
            "Question: {}"
        )
    ...
    try:
        ...
        if not args.skip_base:
            ...
            if args.lora_adapter_dir is not None:
                _manual_merge_lora(base_model, args.lora_adapter_dir)
                base_model.eval()
                if context.rank == 0:
                    print(f"[locomo] manually merged LoRA adapter {args.lora_adapter_dir} into base model", flush=True)
            if args.peft_adapter_dir is not None:
                from peft import PeftModel

                base_model = PeftModel.from_pretrained(base_model, str(args.peft_adapter_dir)).eval()
                if context.rank == 0:
                    print(f"[locomo] attached PEFT adapter {args.peft_adapter_dir} via PeftModel.from_pretrained", flush=True)
