...
def load_base_model_and_tokenizer(
    *,
    ...
    import os as _os
    _dmap = "auto" if _os.environ.get("DELTAMEM_DEVICE_MAP") == "auto" else {"": device}
...
def load_peft_model_and_tokenizer(
    *,
    ...
    if os.environ.get("DELTAMEM_LORA_MANUAL_MERGE") == "1":
        _manual_merge_lora(model, Path(adapter_dir))
        model = model.eval()
    else:
...
def _manual_merge_lora(model, adapter_dir: Path) -> None:
    """Merge a PEFT LoRA adapter into base weights WITHOUT peft.from_pretrained
    (broken on this peft/transformers combo). delta_W = (alpha/r) * B @ A."""
    import re as _re
    ...
    for key, value in sd.items():
        m = _re.match(r"(.*)\.lora_(A|B)\.weight$", key)
        if m:
    ...
    for base_key, ab in pairs.items():
        if "A" not in ab or "B" not in ab:
            continue
        module = model.get_submodule(base_key.replace("base_model.model.", ""))
