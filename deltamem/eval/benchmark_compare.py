...
def normalize_hotpotqa_answer(text: str) -> str:
    # EXACT port of the official hotpot_evaluate_v1.normalize_answer: lower -> DELETE
    # punctuation (not replace-with-space) -> drop articles -> collapse whitespace.
    # (normalize_qa_span, used by MemoryAgentBench, REPLACES punct with space and must not
    # change; HotpotQA needs the official deletion so "U.S."->"us", "Spider-Man"->"spiderman".)
    text = text.lower()
...
def hotpotqa_f1(prediction: str, gold: str) -> float:
    pred = normalize_hotpotqa_answer(prediction)
    gold_n = normalize_hotpotqa_answer(gold)
    # official yes/no/noanswer special case: if either side is a yes/no/noanswer token and
    # they don't match exactly, F1 is 0 (token overlap must not give partial credit here).
    if pred in ("yes", "no", "noanswer") and pred != gold_n:
        return 0.0
    if gold_n in ("yes", "no", "noanswer") and pred != gold_n:
        return 0.0
...
def _shared_prefix_snapshot_device(prefix_token_count: int, *, device: str, keep_on_gpu: bool = False) -> str:
    if not device.startswith("cuda"):
        return device
    # Under device_map sharding the KV cache is split per-layer across GPUs. Collapsing it to a
    # single device (CPU/cuda:0) and restoring it there breaks the per-layer device placement,
    # causing cross-device cat errors. Keep it on-GPU so clone preserves each layer's device.
    if os.environ.get("DELTAMEM_DEVICE_MAP") == "auto":
        return device
...
def _shared_prefix_forward(
    ...
    *,
    ...
    # logits_to_keep=1 makes the model materialize logits ONLY for the last position instead of
    # all N positions (N*vocab*2 bytes can be 70+GB for long contexts -> OOM). We only ever use
    # the last-token logits here. Fall back gracefully if the model signature lacks the arg.
    try:
        return model(
            input_ids=input_ids,
            past_key_values=past_key_values,
...
def _memory_agent_bench_recompute_metrics(
    ...
    *,
    ...
    os.chdir(os.environ.get("MAB_SCORE_CWD", str(repo_root)))
    try:
